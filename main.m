...
%     legend('x_{L}','y_{L}','z_{L}')
    ...
    for s = 1:25 % iterate through potential segments (there are 25 in the calibration
        ...
        z = (sqrt((velocities(int,7).^2)+(velocities(int,8).^2)+(velocities(int,9).^2))+sqrt((velocities(int,13).^2)+(velocities(int,14).^2)+(velocities(int,15).^2)))/2; % compute the average speed of the hands
        threshold = 0.003; % threshold for z where movement is considered as motion rather than noise
